...
def process_parquets_silver(spark):
    ...
    for ds in datasets:
        try:
            ...
            output_path_hdfs = f"hdfs://namenode:9000/data/silver/{ds}.parquet"
            df.write.mode("overwrite").parquet(output_path_hdfs)

            if ds == "iris":
                # Sauvegarde dans MongoDB Atlas (dataarchi database, location collection)
                print(f" -> Sauvegarde de {ds} dans MongoDB Atlas (dataarchi.location)")
                mongo_uri = f"mongodb+srv://{mongo_user}:{mongo_password}@cluster0.3bidwmj.mongodb.net/"
                df.write.format("mongo").mode("overwrite").option("spark.mongodb.write.connection.uri", mongo_uri).option("spark.mongodb.write.database", "dataarchi").option("spark.mongodb.write.collection", "location").option("writeConcern.wtimeout", 0).save()
            else:
                # Sauvegarde dans Hive
                print(f" -> Sauvegarde de {ds} dans Hive")
                table_name = ds.replace("-", "_")
                spark.sql("CREATE DATABASE IF NOT EXISTS silver")
                df.write.mode("overwrite").saveAsTable(f"silver.{table_name}")

            print(f" -> OK: {ds} traité avec succès")
